...
int main(void) {
    ...
    CHECK(crv_status_string(CRV_ERR_STALE) != NULL);
    ...
    CHECK(crv_node_unary(ir, CRV_OP_ADD, xr, &bad) == CRV_ERR_INVALID_ARGUMENT);
...

    {
        ...
        /* Growing the IR invalidates the solver bound to it. */
        CHECK(crv_node_const_u64(ir, 0, 4, &bad) == CRV_OK);
        CHECK(crv_solver_next(solver, values, words) == CRV_ERR_STALE);
    }
    ...
    /* Hash, serialize, reload, and confirm the reload is the same query. */
    {
        ...
        /* A damaged blob is a status, not a crash. */
        blob[size / 2] ^= 0xff;
        loaded = ir;
